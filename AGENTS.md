# AGENTS — Site Astro, cadre pédagogique et éditorial
...
- **Tout statique et local** : aucun CDN, aucune ressource externe au runtime
  ...
  **Seconde exception explicite** (décision auteur, septembre 2026) : toute
  vidéo YouTube citée utilise uniquement le composant `<VideoEmbed>` (jamais
  une simple miniature, un lien « voir sur YouTube » ou une iframe écrite à
  la main dans le MDX). Le lecteur `youtube-nocookie.com` est chargé
  directement, sans écran de consentement, à `100 %` de la largeur de sa
  colonne. Les playlists utilisent `playlistId`.
...
- **Glossaire systématique** : les termes techniques, anglicismes
  ...
  avec `<G t="slug">`.
- **Fiche obligatoire pour chaque personnage cité** : toute personne réelle
  nommée dans un cours, un article, une légende ou une ressource reçoit dans
  le même changement une entrée bilingue `type: "personnage"` dans
  `site/src/data/glossaire.mjs`. Cette entrée constitue sa fiche minimale ;
  ajouter une fiche longue MDX FR/EN lorsque le contexte demande davantage
  qu'un bref.
- **Traçabilité du glossaire** : toute notion ajoutée doit être citée ou
  enseignée sur le site. Renseigner `cours` lorsqu'elle est enseignée dans un
  module et conserver des formes/alias assez précis pour éviter les faux
  positifs. Les fiches doivent conserver leurs liens retour « Cité dans le
  site » vers tous les cours et articles où elles sont repérées.
- **Ressources du glossaire** : les vidéos et les autres liens externes cités
  dans les contenus sont recensés automatiquement dans les sections
  Ressources du glossaire ; ne jamais dupliquer manuellement cette liste.
  Les liens Wikipédia/Wikimedia propres aux fiches ne sont pas répétés dans
  cette liste générale.
- **Provenance humain / IA obligatoire** : chaque cours publié, FR et EN,
  affiche sa jauge dans son bandeau. Un module hérite de sa série ; un cours
  autonome déclare toujours `provenance` et un `aiShare` explicite entre 0 et
  100 dans son frontmatter, même s'il est en construction. Toutes les pages
  générées doivent contenir `.prov__gauge`.
...
npm run check      # contenu + build + smoke tests, obligatoire avant de conclure
npm run dev        # aperçu local
```

Le contrôle doit notamment garantir les slugs de `<G>`, les modules déclarés
par `cours`, les liens retour des fiches et les jauges humain/IA. Vérifier sur
la page rendue : maths KaTeX, composants câblés (exercice verrouillé,
terminal, diaporama) et aucune ressource réseau externe hors exceptions
Wikimedia Commons et `VideoEmbed`.
