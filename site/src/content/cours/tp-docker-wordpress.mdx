...
## Dockerfile : du Dockerfile au fichier docker-compose par l'exemple
...
- Si vous n'avez besoin de changer de chemin que pour une ou des commandes, n'utilisez pas le mot-clé `WORKDIR` (cela ajouterait une image intermédiaire inutile). Préférez l'utilisation de :
...
- Vous allez exécuter un conteneur contenant plusieurs services : je vous propose de lire cet [article](https://docs.docker.com/engine/admin/multi-service_container/).
...
Tout d'abord, un peu de [lecture](https://docs.docker.com/engine/security/security) sur la sécurité lorsque l'on utilise un environnement Docker, ou bien directement la [conclusion](https://docs.docker.com/engine/security/security/#conclusions).
...
<Note type="warning" title="Nommez vos volumes">
  Lorsque vous utilisez la méthode 1, vous devez [nommer vos volumes](https://docs.docker.com/storage/volumes/#remove-volumes). En effet, les volumes anonymes sont supprimés à la suppression du conteneur ou lors de l'utilisation de la commande `docker volume prune`.
</Note>

<Note type="danger" title="Attention">
  Si un volume nommé n'est utilisé par aucun conteneur, alors la commande `docker volume prune` supprimerait ce volume, bien que celui-ci soit nommé.
</Note>

<Note title="N.B.">
  Si vous devez gérer des données temporaires, il vaut mieux utiliser la directive [`--tmpfs`](https://docs.docker.com/storage/tmpfs/) plutôt que d'utiliser la couche en écriture de votre conteneur.
</Note>
...
- Bien que dépréciée, vous utiliserez ici l'option historique `--link` pour lier vos 2 conteneurs.
...
### Étape 6 : une architecture de microservices décrite dans un fichier docker-compose
...
- Remplacez votre script `build_and_launch.sh` par un fichier docker-compose en version 3 qui vous permette d'avoir une configuration équivalente au script.
...
### Étape 7 : une architecture de microservices décrite dans un fichier docker-compose fourni par la communauté
...
- Utilisation d'un fichier au format docker-compose pour le déploiement (modèle déclaratif).
...
Déployer une pile (*stack*) d'une application de vote (*voting app*) avec Docker Swarm. Le but de ce TP est d'illustrer comment déployer une pile (application multi-(micro)services) avec Swarm et à partir d'un fichier docker-compose.
...
En fonction de ce qui a été défini dans le fichier docker-compose et des éventuelles mises à jour disponibles, vous pouvez mettre à jour la pile (le service dans sa globalité) de cette manière :
